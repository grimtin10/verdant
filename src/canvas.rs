...
use crate::{GpuContext, RendererResult, Vertex, image::Image, ortho, render_surface::RenderSurface, shape_vertices::{canvas_vertices, ellipse_vertices, line_vertices, rect_vertices, textured_vertices}, shapes::{ScalingMode, Style}, text::{self, Font, HorizontalAlign, Span, TextLayout, VerticalAlign}, transform::{GpuTransform2d, Transform2d}, types::{Color, TextLayoutCache}, vec::Vec2, view::{View, ViewMode}};
...
impl RenderSurface for CanvasDraw {
...
impl Canvas {
    ...
    /// Returns a read guard for the inner [`CanvasDraw`], locking it until the guard is dropped.
    pub fn read(&self) -> RwLockReadGuard<'_, CanvasDraw> {
    ...
    /// Returns a write guard for the inner [`CanvasDraw`], locking it until the guard is dropped.
    pub fn write(&self) -> RwLockWriteGuard<'_, CanvasDraw> {
    ...
    /// Acquires a write guard on the inner [`CanvasDraw`], then running `commands` with it passed
    /// as an argument, locking it until `commands` is finished running.
    ///
    /// In multithreaded contexts, this function prevents the canvas from being flushed until it
    /// completes, meaning that whatever is flushing it will wait until you are finished drawing.
    pub fn draw(&self, commands: impl FnOnce(&mut CanvasDraw)) {
        commands(&mut self.write())
    }

    /// Set whether or not this canvas will be implicitly flushed whenever composited onto a [`RenderSurface`].
    /// Defaults to `true`.
    /// If set to `false`, you must manually flush the canvas when you are finished drawing.
    pub fn auto_flush(&self, auto_flush: bool) {
        self.write().auto_flush = auto_flush;
    }
}
